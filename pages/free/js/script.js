...
function renderResults(data) {
  ...
  data.forEach(item => {
    ...
    card.className = "dark:bg-gray-800 dark:border-gray-900 dark:text-white border rounded-lg shadow-sm p-3 mb-3 bg-white transition";

    card.innerHTML = `
      <div class="flex gap-3 text-white dark:bg-gray-800 dark:text-white">
        <img 
  ...
  />
        <div class="flex-grow">
          <p><span class="font-bold">Offers:</span> ${item.offer} 
            ${item.verified ? '<span class="text-green-600 text-xs ml-1">✔</span>' : ""}
          </p>
          <p><span class="font-bold">Wants:</span> ${item.want}</p>
          <p class="text-xs text-gray-500 dark:text-blue-300">📍 ${item.location} · 🗓️ ${item.date}</p>
          <p class="text-sm text-gray-700 mt-1 dark:text-white"><span class="font-bold">Message:</span> ${item.message}</p>
        </div>
      </div>
      <div class="flex justify-between mt-3">
        <button class="cancelBtn hidden border border-red-300 text-red-500 px-3 py-1 rounded text-sm dark:border-gray-500">Cancel</button>
